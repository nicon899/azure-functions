...
(
	...
	(
		...
		ON ( t.ID = obj.OBJECT_TYPE_ID AND t.NAME = 'Product Family')
	) a
	ON v.OBJECT_ATTRIBUTE_ID = a.ID_OBJ_ATTR
	WHERE v.[VALUE] is not NULL 
) attr PIVOT(MAX(attr.[VALUE]) FOR attr.attrName IN ( [Key] , [Name] , [Product Line] , [Created] , [Updated])) AS pt_TEXT_VALUE