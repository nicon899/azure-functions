...
(
	...
	(
		...
		ON ( t.ID = obj.OBJECT_TYPE_ID AND t.NAME = 'Contact')
	) a
	ON v.OBJECT_ATTRIBUTE_ID = a.ID_OBJ_ATTR
	WHERE v.[VALUE] is not NULL 
) attr PIVOT(MAX(attr.[VALUE]) FOR attr.attrName IN ( [Key] , [First Name] , [Last Name] , [Email] , [Company] , [Created] , [Updated])) AS pt_TEXT_VALUE